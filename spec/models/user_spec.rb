...
describe User do
  ...
  context 'refresh folder' do
    ...
    it 'refreshes a feed' do
      ...
      FeedClient.stub(:fetch) do |fetch_id|
        if fetch_id == @feed1.id
          @feed1.entries << entry3
        elsif fetch_id == @feed2.id
          @feed2.entries << entry4
        end
      end

      @feed1.entries.count.should eq 1
      @feed2.entries.count.should eq 1

      @user.refresh_folder @folder.id

      @feed1.entries.count.should eq 2
      @feed1.entries.should include @entry1
      @feed1.entries.should include entry3
      @feed2.entries.count.should eq 2
      @feed2.entries.should include @entry2
      @feed2.entries.should include entry4
    end

    it 'refreshes all subscribed feeds' do
      # @user is subscribed to feed3, which is not in any folder
      feed3 = FactoryGirl.create :feed
      @user.feeds << feed3

      entry3 = FactoryGirl.build :entry, feed_id: @feed1.id
      entry4 = FactoryGirl.build :entry, feed_id: @feed2.id
      entry5 = FactoryGirl.build :entry, feed_id: feed3.id
      FeedClient.stub(:fetch) do |fetch_id|
        if fetch_id == @feed1.id
          @feed1.entries << entry3
        elsif fetch_id == @feed2.id
          @feed2.entries << entry4
        elsif fetch_id == feed3.id
          feed3.entries << entry5
        end
      end

      @feed1.entries.count.should eq 1
      @feed2.entries.count.should eq 1
      feed3.entries.count.should eq 0

      @user.refresh_folder 'all'
      ...
      @feed2.entries.should include @entry2
      @feed2.entries.should include entry4
      feed3.entries.count.should eq 1
      feed3.entries.should include entry5
    end

    it 'returns unread entries from the folder' do
      entry3 = FactoryGirl.build :entry, feed_id: @feed1.id
      entry4 = FactoryGirl.build :entry, feed_id: @feed2.id
      FeedClient.stub(:fetch) do |fetch_id|
        if fetch_id == @feed1.id
      ...
      entries = @user.refresh_folder @folder.id
    ...
    it 'returns unread entries from all subscribed feeds' do
      # @user is subscribed to feed3, which is not in any folder
      feed3 = FactoryGirl.create :feed
      @user.feeds << feed3
      ...
      entries = @user.refresh_folder 'all'

      entries.count.should eq 4
      entries.should include @entry2
      entries.should include entry3
      entries.should include entry4
      entries.should include entry5
    end

    it 'raises an error trying to refresh a folder that does not belong to the user' do
      expect {@user.refresh_folder 1234567890}.to raise_error ActiveRecord::RecordNotFound
    end
  end
