...
describe User do
  ...
  context 'add feed to folder' do
    ...
    it 'adds a feed to a folder' do
      ...
      @user.move_feed_to_folder @feed.id, @folder.id
      @folder.reload
    ...
    it 'removes the feed from its old folder' do
      ...
      @user.move_feed_to_folder @feed.id, folder2.id
    ...
    it 'returns the feed' do
      folders = @user.move_feed_to_folder @feed.id, @folder.id
    ...
    it 'returns the old folder' do
      ...
      folders = @user.move_feed_to_folder @feed.id, folder2.id
    ...
    it 'raises an error if the folder does not belong to the user' do
      ...
      expect {@user.move_feed_to_folder @feed.id, folder.id}.to raise_error ActiveRecord::RecordNotFound
    end
