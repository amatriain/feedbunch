...
describe User do
  ...
  context 'add feed to new folder' do
    ...
    it 'creates new folder' do
      @user.move_feed_to_folder @feed.id, folder_title: @title
    ...
    it 'returns the new folder' do
      changed_data = @user.move_feed_to_folder @feed.id, folder_title: @title
      folder = @user.folders.where(title: @title).first
    ...
    it 'raises an error if the user already has a folder with the same title' do
      ...
      @user.folders << folder
      expect {@user.move_feed_to_folder @feed.id, folder_title: @title}.to raise_error FolderAlreadyExistsError
    end
    ...
    it 'does not raise an error if another user has a folder with the same title' do
      ...
      expect {@user.move_feed_to_folder @feed.id, folder_title: @title}.to_not raise_error
    end

    it 'raises an error if user is not subscribed to the feed' do
      feed2 = FactoryGirl.create :feed
      expect {@user.move_feed_to_folder feed2.id, folder_title: @title}.to raise_error ActiveRecord::RecordNotFound
    end
  end
end
