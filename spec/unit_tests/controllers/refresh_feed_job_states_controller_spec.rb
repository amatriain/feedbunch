...
describe Api::RefreshFeedJobStatesController, type: :controller do
  ...
  context 'GET show' do
    ...
    it 'returns a 404 for a job state that does not belong to the user' do
      job_state_3 = FactoryBot.create :refresh_feed_job_state
  ...
  context 'DELETE remove' do
    ...
    it 'deletes the job state' do
      delete :destroy, params: {id: @job_state_1.id}, format: :json
      expect(RefreshFeedJobState.exists?(@job_state_1.id)).to be false
    end
    ...
    it 'returns 500 if there is a problem unsubscribing' do
      allow_any_instance_of(RefreshFeedJobState).to receive(:destroy).and_raise StandardError.new
      delete :destroy, params: {id: @job_state_1.id}, format: :json
