...
describe SubscribeUserWorker do
  ...
  it 'subscribes user to already existing feeds' do
    ...
    SubscribeUserWorker.new.perform @user.id, @feed.fetch_url, @folder.id, false, nil
    @user.reload
  ...
  it 'creates new feeds and subscribes user to them' do
    ...
    SubscribeUserWorker.new.perform @user.id, @url, @folder.id, false, nil
    @user.reload
  ...
  context 'validations' do
    ...
    it 'does nothing if the job_status is not in state RUNNING' do
      ...
      expect(@user).not_to receive :subscribe
      SubscribeUserWorker.new.perform @user.id, @feed.fetch_url, @folder.id, false, job_state.id
    end
