...
describe OpmlImportJobState, type: :model do
  ...
  context 'validations' do
    ...
    it 'always belongs to a user' do
      opml_import_job_state = FactoryBot.build :opml_import_job_state, user_id: nil
  ...
  context 'default values' do
    ...
    it 'defaults to zero total_feeds' do
      opml_import_job_state = FactoryBot.create :opml_import_job_state, total_feeds: nil
      expect(opml_import_job_state.total_feeds).to eq 0
    end

    it 'defaults to zero processed_feeds' do
      opml_import_job_state = FactoryBot.create :opml_import_job_state, processed_feeds: nil
      expect(opml_import_job_state.processed_feeds).to eq 0
    end

    it 'defaults show_alert to true' do
      opml_import_job_state = FactoryBot.build :opml_import_job_state, show_alert: nil
  ...
  context 'relationship with OpmlImportFailures' do
    ...
    before :each do
      @job_state = FactoryBot.create :opml_import_job_state
    end
    ...
    it 'does not destroy OpmlImportFailure instances associated with other jobs' do
      ...
      import_failure_1 = FactoryBot.build :opml_import_failure, opml_import_job_state_id: job_state_2.id
      import_failure_2 = FactoryBot.build :opml_import_failure, opml_import_job_state_id: job_state_2.id
